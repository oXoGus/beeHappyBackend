...
router.get("/:param", (req, res) => {
  ...
  connection.connect((err) => {
        ...
        connection.query("SELECT weight, messageDateTime FROM ruche1 ORDER BY messageDateTime DESC", (err, rows) => {
            if(err) throw err;
            connection.end()
            for(let i=0; i < param; i++){
                data.push(rows[i]); // on cree la nouvelle liste 
            }
            res.json(data)
        })
        console.log("connection a la base de donn  e r  ussi !")
    });
})
